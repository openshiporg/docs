...
// Raw Prisma schema content (copied from openfront schema.prisma)
const PRISMA_SCHEMA = `
model Address {
...
function parseModelFields(modelBody: string): PrismaField[] {
  ...
  for (const line of lines) {
    ...
    // Skip relation arrays and special fields
    if (fieldDefinition.includes('[]') && fieldDefinition.includes('@relation')) {
      continue;
    }
    
    const field: PrismaField = {
      name: fieldName,
      type: extractPrismaType(fieldDefinition),
    };
    ...
    // Check if it's a foreign key (has references or ends with Id and has @relation or @map)
    if (fieldDefinition.includes('references:') || 
        fieldDefinition.includes('@relation') || 
        (fieldName.endsWith('Id') && (fieldDefinition.includes('@map') || fieldDefinition.includes('?')))) {
...
function extractPrismaType(fieldDefinition: string): string {
  // Extract the base type from the field definition
  const typeMatch = fieldDefinition.match(/^(\w+)(\?)?/);
  if (!typeMatch) return 'unknown';
  
  const baseType = typeMatch[1];
  
  // Map Prisma types to database types
  ...
  return typeMapping[baseType] || baseType.toLowerCase();
}

function calculateNodePositions(models: PrismaModel[], edges: Edge[]): Record<string, { x: number, y: number }> {
  ...
  models.forEach((model) => {
    const nodeWidth = 280;
    const nodeHeight = Math.max(200, model.fields.length * 30 + 80); // Dynamic height based on fields
    
    dagreGraph.setNode(model.name, {
      width: nodeWidth,
      height: nodeHeight,
    });
  });
  ...
  // Extract positions
  models.forEach((model) => {
    const node = dagreGraph.node(model.name);
    if (node) {
      positions[model.name] = {
        x: node.x - node.width / 2,  // Dagre uses center, ReactFlow uses top-left
...
function generateEdges(models: PrismaModel[]): Edge[] {
  const edges: Edge[] = [];
  const edgeSet = new Set<string>();
  
  // Create a map of model names to their primary key fields
  const primaryKeys: Record<string, string> = {};
  models.forEach(model => {
    const primaryField = model.fields.find(f => f.isPrimary);
    if (primaryField) {
      primaryKeys[model.name] = primaryField.name;
    }
  });
  
  // 1. Parse direct relationships with fields/references
  const directRelationRegex = /(\w+)\s+(\w+)(\??)\s+@relation\("([^"]+)",\s*fields:\s*\[([^\]]+)\],\s*references:\s*\[([^\]]+)\]/g;
  
  let match;
  while ((match = directRelationRegex.exec(PRISMA_SCHEMA)) !== null) {
    const [, fieldName, relatedModelName, optional, relationName, fieldsStr, referencesStr] = match;
    
    // Find the model that contains this relationship field
    const sourceModel = models.find(model => {
      const modelRegex = new RegExp(`model\\s+${model.name}\\s*{[^}]*${fieldName}\\s+${relatedModelName}`, 's');
      return modelRegex.test(PRISMA_SCHEMA);
    });
    
    const targetModel = models.find(m => m.name === relatedModelName);
    
    if (sourceModel && targetModel && primaryKeys[targetModel.name]) {
      const fields = fieldsStr.split(',').map(f => f.trim());
      const references = referencesStr.split(',').map(r => r.trim());
      
      // Create edge for each field-reference pair
      fields.forEach((field, index) => {
        const reference = references[index] || references[0];
        const edgeId = `${targetModel.name}-${sourceModel.name}-${field}`;
  ...
  // 2. Parse many-to-many and reverse relationships (arrays)
  const reverseRelationRegex = /(\w+)\s+(\w+)\[\]\s+@relation\("([^"]+)"\)/g;
  
  while ((match = reverseRelationRegex.exec(PRISMA_SCHEMA)) !== null) {
    const [, fieldName, relatedModelName, relationName] = match;
    
    // Find the model that contains this array relationship field
    const sourceModel = models.find(model => {
      const modelRegex = new RegExp(`model\\s+${model.name}\\s*{[^}]*${fieldName}\\s+${relatedModelName}\\[\\]`, 's');
      return modelRegex.test(PRISMA_SCHEMA);
    });
    ...
    if (sourceModel && targetModel && primaryKeys[sourceModel.name] && primaryKeys[targetModel.name]) {
      const edgeId = `${sourceModel.name}-${targetModel.name}-${relationName}`;
      ...
      if (!edgeSet.has(edgeId)) {
        ...
        edges.push({
          ...
          sourceHandle: primaryKeys[sourceModel.name],
          targetHandle: primaryKeys[targetModel.name],
          type: "custom",
        });
      }
    }
  ...
  // 3. Additional fallback for simple patterns that might not have been caught
  models.forEach(model => {
    model.fields.forEach(field => {
      if (field.isForeign && field.name.endsWith('Id')) {
        // Pattern: userId -> User, shopId -> Shop, etc.
        const baseName = field.name.slice(0, -2);
        const sourceModel = models.find(m => m.name.toLowerCase() === baseName.toLowerCase());
        
        if (sourceModel && primaryKeys[sourceModel.name]) {
          const edgeId = `${sourceModel.name}-${model.name}-${field.name}`;
...
function generateSchemaData(): { nodes: Node[], edges: Edge[] } {
  const models = parsePrismaSchema(PRISMA_SCHEMA);
  
  // Generate edges first since we need them for positioning
  const edges = generateEdges(models);
  
  // Calculate positions using dagre with the edges
  const positions = calculateNodePositions(models, edges);
...
export const { nodes: initialNodes, edges: initialEdges } = generateSchemaData();