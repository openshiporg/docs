---
name: openfront-barbershop-custom-storefront
description: Plan and build a custom Openfront Barbershop storefront by explaining the source architecture, interviewing the user, inspecting the current product code and contract, then adapting the built-in client or creating a safe separate client.
---

# Openfront Barbershop custom storefront

Use this skill to adapt the built-in Barbershop storefront or create a separately deployed client. Start from this product's current source—not Ecommerce conventions, screenshots, or another vertical's schema.
...
- Built-in Barbershop customer experience, screens/modules, data loaders, sessions, and UI composition: `features/storefront`.
...
The current product focus is service and barber discovery, availability, booking, customer access, packages/memberships, and appointment state. This list is orientation, not proof that every flow is safe or complete.
