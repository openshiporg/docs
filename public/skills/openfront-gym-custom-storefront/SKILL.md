---
name: openfront-gym-custom-storefront
description: Plan and build a custom Openfront Gym storefront by explaining the source architecture, interviewing the user, inspecting the current product code and contract, then adapting the built-in client or creating a safe separate client.
---

# Openfront Gym custom storefront

Use this skill to adapt the built-in Gym storefront or create a separately deployed client. Start from this product's current source—not Ecommerce conventions, screenshots, or another vertical's schema.
...
- Built-in Gym customer experience, screens/modules, data loaders, sessions, and UI composition: `features/storefront`.
...
The current product focus is plans/classes, availability and booking/waitlist, member account, trainer discovery, and check-in code. This list is orientation, not proof that every flow is safe or complete.
