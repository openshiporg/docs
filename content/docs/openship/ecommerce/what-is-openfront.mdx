...
description: Introducing Openship, an order routing platform that connects your sales channels to fulfillment partners.
...
Channels represent destinations where your orders can be routed and fulfilled. A channel could be an existing platform like your supplier's Shopify shop or a 3PL fulfillment service. It could also be something very simple like adding a row to a Google Sheet or sending an email with the order details. Openship currently only supports Shopify out of the box, but custom channels can be created. Check out [this guide](/docs/openship/ecommerce/how-to-guides/create-custom-channel) to learn more.
